...
forecast_calibration(eval_forecasts, percent_bin = 0.1, ssn_group = TRUE,
                     tgt_group = TRUE, team_group = T) %>%
  ggplot() + 
  geom_point(aes(x = mean_pred_prob, y = percent, color = team),
               size = 1.4) +
  geom_errorbar(aes(x = mean_pred_prob, ymin = lower, ymax = upper,
                    width = width/4, color = team)) +
  # Add reference line
...
```{r expand calibration plots}
forecast_calibration(eval_forecasts_exp, percent_bin = 0.1, exact = FALSE,
                     ssn_group = TRUE, tgt_group = TRUE) %>%
  ggplot() + 
...
eval_forecasts %>%
  ...
  summarize(mean = mean(encompass_true)) %>%
  spread(key = "team", value = "mean") %>%
  arrange(season, target)
