...
#   filter(!season %in% c("2008/2009", "2009/2010", "2018/2019")) %>%
...
save(transform_scores, file = "Data/CV_Transform_Scores_1617.Rdata")
...
best_single_transform_cv <- transform_scores %>%
  group_by(model) %>%
  summarize(avg_score = mean(score)) %>%
  ungroup() %>%
  filter(avg_score == max(avg_score)) %>%
  select(transform = model) %>%
  crossing(location = unique(transform_scores$location))

save(best_transform_cv, file = "Data/CV_Transform_terms_1617.Rdata")
save(best_single_transform_cv, file = "Data/CV_Single_Transform_term_1617.Rdata")

##### Number of Fourier terms #####
load("Data/fourier_fits_1617.Rdata")
...
save(fourier_model_fits, file = "Data/fourier_fits_1617.Rdata")
...
for (this_season in c("2010/2011", "2011/2012", "2012/2013", "2013/2014",
  ...
  save(fourier_scores, file = "Data/CV_Fourier_Scores_1617.Rdata")
...
save(best_k_cv, file = "Data/CV_Fourier_terms_1617.Rdata")

##### ARIMA structure for error terms #####

load("Data/CV_Fourier_terms_1617.Rdata")
load("Data/CV_Transform_terms_1617.Rdata")
load("Data/arima_fits_1617.Rdata")
...
save(arima_model_fits, file = "Data/arima_fits_1617.Rdata")
...
for(this_season in c(# "2010/2011", "2011/2012"#,
  ...
  save(arima_scores, file = paste0("Data/CV_ARIMA_Scores_", substr(this_season, 1, 4),
                                   ".Rdata"))
