...
create_subtype_densities <- function(ili_df, 
                                     vir_ssn_per, 
                                     vir_wk_per,
                                     pseudo_onsets = c(),
                                     challenge = "ilinet",
  ...
  for(this_location in unique(train_ili$location)) {
    ...
    if(isTRUE(seasonal)) {
      ...
      if(challenge == "ilinet") {
        ...
        densities[["b"]][["Season onset"]][[this_location]] <- 
          density(temp_onset$bin_start_incl, bw = "SJ",
                  weights = temp_onset$bper / sum(temp_onset$bper))
      }
      ...
      out_h3 <- try(density(temp_peak_wk$bin_start_incl, bw = "SJ",
                    ...
                    silent = TRUE)
      out_b <- try(density(temp_peak_wk$bin_start_incl, bw = "SJ",
                            weights = temp_peak_wk$bper / sum(temp_peak_wk$bper)),
                    silent = TRUE)
      ...
      if(!is.character(out_b))
        densities[["b"]][["Season peak week"]][[this_location]] <- out_b
      ...
      out_h3 <- try(density(temp_peak_per$bin_start_incl, bw = "SJ",
                    ...
                    silent = TRUE)
      out_b <- try(density(temp_peak_per$bin_start_incl, bw = "SJ",
                            weights = temp_peak_per$bper / sum(temp_peak_per$bper)),
                    silent = TRUE)
      ...
      if(!is.character(out_b))
        densities[["b"]][["Season peak percentage"]][[this_location]] <- out_b
    ...
    for(this_week in 40:77) {
      ...
      # For national/regional, join 6 week lag
      # For states, join cumulative by date
      if(challenge == "ilinet") {
        temp_ili <- filter(train_ili, location == this_location,
                           order_week == this_week) %>%
          left_join(vir_wk_per, by = c("season", "location", "order_week")) %>%
          filter(!is.na(h1per))
      } else {
        temp_ili <- filter(train_ili, location == this_location,
                           order_week == this_week) %>%
      ...
      out_b <- try(density(temp_ili$ILI, bw = "SJ",
                            weights = temp_ili$bper / sum(temp_ili$bper)),
                    silent = TRUE)
      
      # Save densities to list
      if(is.character(out_h1) & is.character(out_h3) & is.character(out_b)) {
        next
      } else {
        if(!is.character(out_h1))
        ...
        if(!is.character(out_b))
          densities[["b"]][[paste(this_week)]][[this_location]] <- out_b
      }
    }
  }
  ...
  for(this_location in names(functions[["h1"]][[2]])) {
    ...
    if(isTRUE(seasonal)) {
      ...
      if(challenge == "ilinet") {
        ...
        prob_no <- prob_no_onset$h1_prob_no_onset[prob_no_onset$location == this_location] * temp_vir$h1per_ssn + 
          prob_no_onset$h3_prob_no_onset[prob_no_onset$location == this_location] * temp_vir$h3per_ssn
    ...
    # Week ahead forecasts, using 6 week rolling for nat/reg and season cum for states
    if(challenge == "ilinet") {
