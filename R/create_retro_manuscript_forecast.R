...
load("Data/CV_covar_terms_1516.Rdata")

fits <- filter(flu_data_merge, year <= 2015,
               season != "2015/2016") %>%
  # Nest by location
  nest(-location) %>%
  # Create time series of ILI
...
forecasts_1516 <- crossing(season = "2015/2016",
                           week = 40:71,
                           location = unique(flu_data_merge$location)) %>%
  filter(week < 71 | season == "2014/2015") %>%
  mutate(
...
saveRDS(prospective_scores_1516, file = "Data/prospective_scores_1516.Rds")
